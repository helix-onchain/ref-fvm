    ...
    V: Serialize + DeserializeOwned,
{
    ...
    /// Iterates over the values in the trie and runs a predicate over each leaf value.
    /// If `start_at` is specified, the values are iterated from that leaf value onwards (considering
    /// branches to be sorted via preorder traversal). The `LeafCursor` should be generated by a
    /// previous call to `for_each_range` on the same (unmodified) trie.
    /// If `limit` is specified, the iteration will stop after `limit` values have been traversed.
